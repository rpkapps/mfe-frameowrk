...
Run `pnpm exec vitest run tests/tracer` for lifecycle and transition coverage. Run `pnpm run test:conformance` for the separate revised history/live-state conformance suite. The introductory generated-route fixture also compiles independently through `pnpm run check`.
...
Memory history and jsdom do not prove browser traversal/blockers, shared generated-tree concurrent mounts, federation, real authentication, compiler/build output, CSS/portals, or production performance. Those proofs belong to their dedicated integration suites. Scrolling is mocked because no layout claim is made. See [the validation report](../../docs/gate-zero-validation.md) and [progress](../../docs/progress.md).
