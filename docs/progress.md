...
- The review branch is [`feat/mfe-framework`](https://github.com/rpkapps/mfe-frameowrk/tree/feat/mfe-framework). Original local history remains on `checkpoint/gate-zero-local`. The supplied full specification is now tracked verbatim at [`mfe-framework-spec.md`](./mfe-framework-spec.md); its approved revisions remain in the companion document. No merge, release, package publication, or deployment has occurred.
