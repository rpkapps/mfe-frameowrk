...
The active package surface uses Rsbuild core 2.2.8, with exact 2.1.0 React/Babel plugins and the 2.9.0 Rsbuild MF2 plugin. Rsbuild core 2.2.8 has a narrowly scoped release-age exception for this reviewed build surface; its registry artifact declares no preinstall, install, or postinstall hooks. This exception does not approve dependency scripts: `allowBuilds` remains empty. Rsbuild uses the pinned Rspack 2.2.6 engine underneath. Existing Rspack version references above describe the historical test-shell evidence and remain unchanged.
